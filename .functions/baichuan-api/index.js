...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (configResult.data.length === 0) {
      throw new Error('百川智能配置未找到或未激活');
    }

    const config = configResult.data[0];
    const apiKey = config.api_key;
    const apiBase = config.api_base || 'https://api.baichuan-ai.com';
    const modelId = model || 'Baichuan2-Turbo';

    // 记录请求开始时间
    const startTime = Date.now();

    // 调用百川智能API
    const response = await axios.post(`${apiBase}/v1/chat/completions`, {
      model: modelId,
      messages: messages,
    ...
    // 计算费用
    const costPerToken = 0.000015; // 百川智能的价格
    ...
    await db.collection('ai_conversation_record').add({
      ...
      ai_model_id: 'baichuan2',
      ai_model_name: 'Baichuan2',
      ai_provider: '百川智能',
    ...
    // 更新使用统计
    await updateUsageStats(userId, 'baichuan2', totalTokens, totalCost);
    ...
    console.error('百川智能API调用失败:', error);
    
    // 记录错误日志
    await db.collection('ai_error_logs').add({
      user_id: userId,
      session_id: sessionId,
      error_type: 'baichuan_api_error',
    ...
    return {
      success: false,
      error: error.message || '百川智能API调用失败'
    };
  }
};
