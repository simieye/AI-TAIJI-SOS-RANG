...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (configResult.data.length === 0) {
      throw new Error('零一万物配置未找到或未激活');
    }

    const config = configResult.data[0];
    const apiKey = config.api_key;
    const apiBase = config.api_base || 'https://api.lingyiwanwu.com';
    const modelId = model || 'yi-large';
    ...
    // 计算费用
    const costPerToken = 0.000018; // 零一万物的价格
    ...
    await db.collection('ai_conversation_record').add({
      ...
      ai_model_id: 'yi-large',
      ai_model_name: 'Yi Large',
      ai_provider: '零一万物',
    ...
    // 更新使用统计
    await updateUsageStats(userId, 'yi-large', totalTokens, totalCost);
    ...
    console.error('零一万物API调用失败:', error);
    
    // 记录错误日志
    await db.collection('ai_error_logs').add({
      user_id: userId,
      session_id: sessionId,
      error_type: 'yi_api_error',
    ...
    return {
      success: false,
      error: error.message || '零一万物API调用失败'
    };
  }
};
