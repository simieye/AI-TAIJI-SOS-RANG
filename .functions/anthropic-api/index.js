...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (configResult.data.length === 0) {
      throw new Error('Anthropic配置未找到或未激活');
    }

    const config = configResult.data[0];
    const apiKey = config.api_key;
    const apiBase = config.api_base || 'https://api.anthropic.com';
    const modelId = model || 'claude-3-sonnet-20240229';
    ...
    // 计算费用
    const costPerToken = 0.000025; // Claude-3的价格
    ...
    await db.collection('ai_conversation_record').add({
      ...
      ai_model_id: 'anthropic-claude-3',
    ...
    // 更新使用统计
    await updateUsageStats(userId, 'anthropic-claude-3', totalTokens, totalCost);
